...
bool buttonAisNotPressed = false;
bool buttonBisNotPressed = false;
...
int main()
{
    ...
    while (true)
    {
        ...
        {
        case AND:
            if (buttonAisNotPressed && buttonBisNotPressed)
            {
                setLED(GREEN);
            }
            else
...
void readButtons()
{
    buttonAisNotPressed = gpio_get(BUTTON_A_PIN);
    buttonBisNotPressed = gpio_get(BUTTON_B_PIN);
}